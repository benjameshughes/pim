...
class ProcessVariantImagesListener implements ShouldQueue
{
    ...
    public function handle(ProductVariantImported $event): void
    {
        try {
            ...
            Log::info('Processing ProductVariantImported event', [
            ...
            Log::info('Found image URLs for variant processing', [
                'variant_id' => $variant->id,
                'urls' => $imageUrls,
            ]);

            // Dispatch the image processing job with fresh variant instance
            ProcessVariantImages::dispatch(
                $variant,
                $imageUrls,
                $event->importedData
            )->delay(now()->addSeconds(5)); // Small delay to ensure variant is fully saved

            Log::info("Dispatched variant image processing job for variant: {$variant->sku}", [
                'variant_id' => $variant->id,
                'image_count' => count($imageUrls),
            ]);
        } catch (\Throwable $e) {
            Log::error('Failed to handle ProductVariantImported event', [
