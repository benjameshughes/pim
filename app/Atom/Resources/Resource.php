...
abstract class Resource
{
    ...
    /**
     * Determine if this resource should register navigation.
     */
    ...
    /**
     * Get custom navigation configuration.
     */
    ...
    /**
     * Check if resource has a specific page.
     */
    ...
    /**
     * Resolve a record from route parameter.
     */
    public static function resolveRecord(mixed $key): ?Model
    {
        return static::getEloquentQuery()->find($key);
    }
    
    /**
     * Get sub-navigation items for resource records.
     */
    public static function getRecordSubNavigation(mixed $record = null): array
    {
        return [];
    }
    
    /**
     * Get relationship navigation for resource records.
     */
    public static function getRelationshipNavigation(mixed $record = null): array
    {
        return [];
    }
    ...
    /**
     * Generate URL for a specific page within the resource using Laravel route helper.
     */
    public static function getUrl(string $name = 'index', array $parameters = []): string
    {
        $pages = static::getPages();
        if (!isset($pages[$name])) {
            throw new \Exception("Page [{$name}] not found in resource.");
        }
        
        // Build the route name: resources.{slug}.{page}
        $routeName = 'resources.' . static::getSlug() . '.' . $name;
        
        // Convert model instances to route keys
        $routeParameters = [];
        foreach ($parameters as $key => $value) {
            if ($value instanceof Model) {
                $routeParameters[$key] = $value->getRouteKey();
            } else {
