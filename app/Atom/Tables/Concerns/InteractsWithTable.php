...
namespace App\Atom\Tables\Concerns;
...
trait InteractsWithTable
{
    ...
    public function executeTableAction(string $actionKey, $recordId): void
    {
        ...
        // Get the actual Action object (not just the config)
        $actions = $table->getActions();
        
        foreach ($actions as $action) {
            if ($action->getKey() === $actionKey) {
                $actionCallback = $action->getAction();
                
                if ($actionCallback !== null) {
                    // Resolve the record for the action
                    $record = $table->resolveRecord($recordId);
                    
                    // Execute the action callback with the record and component context
                    $actionCallback($record, $this);
                }
                break;
            }
        }
    ...
    public function executeTableBulkAction(string $actionKey): void
    {
        ...
        // Get the actual BulkAction object (not just the config)
        $bulkActions = $table->getBulkActions();
        
        foreach ($bulkActions as $bulkAction) {
            if ($bulkAction->getKey() === $actionKey) {
                $actionCallback = $bulkAction->getAction();
                
                if ($actionCallback !== null) {
                    // Resolve the selected records for the bulk action
                    $records = collect($this->selectedTableRecords)->map(function ($recordId) use ($table) {
                        return $table->resolveRecord($recordId);
                    });
                    
                    // Execute the bulk action callback with the selected records
                    $actionCallback($records);
                }
                
                // Clear selection if configured to do so
                if ($bulkAction->shouldDeselectRecordsAfterCompletion()) {
