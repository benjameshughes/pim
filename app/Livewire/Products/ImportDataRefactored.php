...
class ImportDataRefactored extends Component
{
    ...
    {
        if (str_contains($error, 'required')) return 'validation';
        if (str_contains($error, 'duplicate')) return 'constraint';
        if (str_contains($error, 'format')) return 'format';
        if (str_contains($error, 'barcode')) return 'barcode';
        return 'general';
    }
    
    private function determineErrorSeverity(string $error): string
    {
        if (str_contains($error, 'required') || str_contains($error, 'duplicate')) return 'high';
        if (str_contains($error, 'format') || str_contains($error, 'barcode')) return 'medium';
        return 'low';
    }
    ...
    {
        return view('livewire.products.import-data-refactored', [
