<x-layouts.app>
    <x-slot:title>{{ $variant->title ?? 'Variant Details' }}</x-slot:title>
            ...
            <nav class="flex mb-8" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            <flux:icon name="chevron-right" class="w-3 h-3 text-gray-400 mx-1" />
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ $variant->sku ?? 'Variant' }}</span>
                        </div>
                    </li>
                </ol>
            </nav>

            {{-- Variant Show Component --}}
            <livewire:variants.variant-show :variant="$variant" />
